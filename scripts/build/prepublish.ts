...
// ── Step 8: Copy MITM utilities (skip tsc, loaded via tsx at runtime) ─────
const mitmSrc = join(ROOT, "src", "mitm");
const mitmDest = join(DIST_DIR, "src", "mitm");
if (existsSync(mitmSrc)) {
  console.log("  📦 Copying MITM utilities (TypeScript source)...");
  cpSync(mitmSrc, mitmDest, { recursive: true });
  console.log("  ✅ MITM utilities copied to dist/src/mitm/");
}
