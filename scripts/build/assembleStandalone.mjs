...
export function assemblePathSanitize(projectRoot, outDir, distDir = ".next") {
  ...
  for (const filePath of sanitizeTargets) {
    ...
    // Next embeds distDir in server.js. Windows builds serialize `.build\next`,
    // which Linux containers interpret as a literal backslash directory. The
    // package always ships POSIX-style dist paths, so normalize the embedded
    // distDir string during standalone assembly.
