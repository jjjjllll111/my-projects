...
test("GET /api/settings/qdrant/embedding-models — returns only active credential provider models", async () => {
  await localDb.createProviderConnection({
    provider: "voyage-ai",
    authType: "apikey",
    name: "Voyage test key",
    apiKey: "pa-test-voyage",
    isActive: true,
  });
  ...
  assert.ok(
    body.models.some((m: any) => m.value === "voyage-ai/voyage-4-large"),
    "should include active Voyage embedding models without embedding in display name"
  );
  assert.ok(
    !body.models.some((m: any) => m.value === "openai/text-embedding-3-small"),
    "should not include providers without active credentials"
  );
});

test("GET /api/settings/qdrant/embedding-models — empty without active embedding credentials", async () => {
  const headers = await createManagementSessionHeaders();
  const req = new Request("http://localhost/api/settings/qdrant/embedding-models", {
    method: "GET",
    headers: Object.fromEntries(headers.entries()),
