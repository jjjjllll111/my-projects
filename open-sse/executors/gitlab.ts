...
const GITLAB_MAX_PROMPT_CHARS = 4000;
...
function buildPrompt(messages: OpenAIMessage[] | undefined): string {
  ...
  // Only send the latest user message - GitLab Code Suggestions is a
  // single-turn completion API, not a chat model. System prompts and
  // conversation history create oversized payloads.
  let prompt = userParts.at(-1) || "";

  if (prompt.length > GITLAB_MAX_PROMPT_CHARS) {
    prompt = prompt.slice(0, GITLAB_MAX_PROMPT_CHARS) + "\n[truncated]";
  }
    ...
    const content =
      typeof firstChoice.text === "string"
        ...
        : typeof firstChoice.message === "object" && firstChoice.message
          ? (typeof (firstChoice.message as JsonRecord).content === "string"
            ? (firstChoice.message as JsonRecord).content as string
            : typeof (firstChoice.message as JsonRecord).text === "string"
              ? (firstChoice.message as JsonRecord).text as string
              : "")
