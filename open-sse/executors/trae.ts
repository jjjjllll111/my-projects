...
function flattenQuery(messages: ChatMessage[]): string {
  ...
  let text = parts.join("\n\n");
  // Trae has query size limits; truncate to avoid session creation failures.
  // Prioritize the latest user message (at the end) over system/history.
  const MAX_QUERY_CHARS = 2000000;
  if (text.length > MAX_QUERY_CHARS) {
    const latestUser = parts[parts.length - 1] || "";
    const budget = MAX_QUERY_CHARS - latestUser.length - 30;
    if (budget > 200 && parts.length > 1) {
      const history = parts.slice(0, -1).join("\n\n");
      text = history.slice(0, budget) + "\n[...]\n\n" + latestUser;
    } else {
      text = text.slice(-MAX_QUERY_CHARS);
    }
  }
