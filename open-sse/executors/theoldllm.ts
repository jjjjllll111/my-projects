...
import type { TheOldLlmConfig } from "../../config/providers/registry/theoldllm/index.ts";
...
// ── Model name mapping ────────────────────────────────────────────────────
// Note: 动态模型发现已强制启用，以下为fallback映射
...
async function directFetch(
  ...
  try {
    // 如果启用了代理池，每次请求随机使用一个代理；pool 失败/限流后重试 free-pool。
    if (proxyConfig?.enabled) {
      return await fetchWithProxy(API_URL, fetchOptions, proxyConfig);
    }

    // 否则使用标准fetch
    return await fetch(API_URL, fetchOptions);
  } finally {
    clearTimeout(timer);
...
export class TheOldLlmExecutor extends BaseExecutor {
  ...
  async execute(input: ExecuteInput): Promise<{
    ...
    // 读取代理池配置
    const config = credentials.providerSpecificData as TheOldLlmConfig | undefined;
    const proxyConfig = {
      enabled: config?.useProxyPool ?? false,
      concurrency: 1,
    };

    if (proxyConfig.enabled) {
      log?.info?.("THEOLDLLM", "Using single random proxy per request");
    }
    ...
    try {
      ...
      let upstream = await directFetch(reqBody, signal, proxyConfig);
      ...
      if (isTokenRejected(upstream.status, finalBody)) {
        ...
        upstream = await directFetch(reqBody, signal, proxyConfig);
