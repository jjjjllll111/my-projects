...
export async function runWithProxyContext(
  ...
  const isVercelRelay = (effectiveProxyConfig as { type?: string })?.type === "vercel" || (effectiveProxyConfig as { type?: string })?.type === "cloudflare";
...
async function patchedFetch(
  ...
  // Cloudflare Relay: same protocol as Vercel relay, just different type name
  if (
    contextProxy &&
    typeof contextProxy === "object" &&
    (contextProxy as { type?: string }).type === "cloudflare"
  ) {
    const cf = contextProxy as { host?: string; relayAuth?: string };
    if (!cf.relayAuth) {
      throw new Error("Cloudflare relay configuration error: missing relayAuth");
    }
    const targetUrl = getTargetUrl(input);
    const relayHeaders = buildCloudflareRelayHeaders(targetUrl, cf.relayAuth);
    const mergedHeaders = new Headers(options?.headers);
    for (const [k, v] of Object.entries(relayHeaders)) mergedHeaders.set(k, v);
    const hostForLogs = proxyUrlForLogs(cf.host ? `https://${cf.host}` : "");
    if (process.env.OMNIROUTE_PROXY_FETCH_DEBUG === "true") {
      console.debug(`[ProxyFetch] Routing via Cloudflare relay: ${hostForLogs}`);
    }
    return await originalFetch(`https://${cf.host}`, {
      ...options,
      headers: mergedHeaders,
      duplex: "half",
    });
