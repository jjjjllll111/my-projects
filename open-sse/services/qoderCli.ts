...
const QODER_DEFAULT_MODEL = "qmodel_latest";
...
export function mapQoderModelToLevel(model: string | null | undefined): string | null {
  ...
  if (tierKeys.has(normalized)) return normalized;
  // Map legacy/specific model names to tier keys
  if (normalized.includes("deepseek-r1") || normalized.includes("deepseek-v4-pro")) return "dmodel";
  if (normalized.includes("deepseek-v4-flash")) return "dfmodel";
  if (normalized.includes("qwen3-max") || normalized.includes("qwen3.7-max"))
    return "qmodel_latest";
  if (
    normalized.includes("qwen3-coder") ||
    normalized.includes("qwen3.7-plus") ||
    normalized.includes("qoder-rome")
  )
    return "qmodel";
  if (normalized.includes("kimi-k2") || normalized.includes("kimi-k2.7")) return "kmodel";
  if (normalized.includes("glm-5") || normalized.includes("glm-5.1")) return "gm51model";
