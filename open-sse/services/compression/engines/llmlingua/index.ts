...
import { LLMLINGUA_MODELS, DEFAULT_LLMLINGUA_MODEL, isApiModel } from "./constants.ts";
...
/** 
 * Resolve the active backend: injected fake (for tests), API backend, or ONNX worker.
 * Offline modelPath takes precedence over API routing.
 */
function resolveBackend(modelId?: string, modelPath?: string): LlmlinguaBackend {
  if (_backend) return _backend;
  
  // Offline model path takes precedence - always use ONNX worker
  if (modelPath) {
    return workerBackend;
  }
  
  // Route to API backend for models not in ONNX registry
...
const LLMLINGUA_SCHEMA: EngineConfigField[] = [
  { key: "enabled", type: "boolean", label: "Enabled", defaultValue: true },
  {
    ...
    options: [], // Populated dynamically by frontend from /api/compression/llmlingua/models
  },
  {
    key: "minTokens",
    type: "number",
...
export const llmlinguaEngine: CompressionEngine = {
  ...
  async applyAsync(
    ...
    try {
      const backend = resolveBackend(backendOpts.model, backendOpts.modelPath);
