...
export type LlmlinguaBackendType = "onnx" | "api";

/**
 * Check if a model ID refers to an API-based model.
 * Models not in LLMLINGUA_MODELS registry are treated as API models
 * (dynamically fetched from /v1/models).
 */
export function isApiModel(modelId: string | undefined): boolean {
  if (!modelId) return false;
  
  const entry = LLMLINGUA_MODELS[modelId];
  if (!entry) {
    // Not in registry → assume API model
    return true;
  }
  
  // In registry → check backendType field (future extension)
  // Currently all registered models are ONNX
  return false;
}