...
export async function getUsageForProvider(
  ...
  switch (provider) {
    ...
    case "gitlab-duo":
      return await getGitLabDuoUsage(accessToken, providerSpecificData);
    default:
...
/**
 * GitLab Duo Usage
 * Requires read_api scope for namespace/plan info.
 * All API calls have 5s timeout to prevent blocking the quota page.
 */
async function getGitLabDuoUsage(
  accessToken?: string,
  providerSpecificData?: JsonRecord
): Promise<JsonRecord> {
  if (!accessToken) {
    return { message: "GitLab Duo: no access token available." };
  }

  try {
    const baseUrl =
      typeof providerSpecificData?.baseUrl === "string" && providerSpecificData.baseUrl
        ? (providerSpecificData.baseUrl as string).replace(/\/$/, "")
        : "https://gitlab.com";
    ...
    // Get user info (fast, always works with read_user scope)
    const userRes = await fetch(`${baseUrl}/api/v4/user`, {
      headers,
      signal: AbortSignal.timeout(TIMEOUT_MS),
    });
    const userData = userRes.ok ? ((await userRes.json()) as JsonRecord) : {};
    const userPlan = typeof userData.plan === "string" ? (userData.plan as string) : null;

    // Check token scopes first (fast endpoint)
    let tokenScopes: string[] = [];
    try {
      const infoRes = await fetch(`${baseUrl}/oauth/token/info`, {
        headers,
        signal: AbortSignal.timeout(TIMEOUT_MS),
      });
      if (infoRes.ok) {
        const infoData = (await infoRes.json()) as JsonRecord;
        if (Array.isArray(infoData.scopes)) {
          tokenScopes = infoData.scopes.filter((s): s is string => typeof s === "string");
        }
      }
    } catch { /* ignore timeout/error */ }

    const hasReadApi = tokenScopes.includes("read_api") || tokenScopes.includes("api");

    // Only fetch namespaces if we have read_api scope (otherwise returns 403)
    let bestPlan = userPlan || null;
    if (hasReadApi) {
      try {
        const nsRes = await fetch(
          `${baseUrl}/api/v4/namespaces?owned_only=false&per_page=20`,
          { headers, signal: AbortSignal.timeout(TIMEOUT_MS) }
        );
        if (nsRes.ok) {
          const planOrder = ["free", "bronze", "silver", "premium", "ultimate"];
          const namespaces = (await nsRes.json()) as JsonRecord[];
          for (const ns of namespaces) {
            const nsPlan = typeof ns.plan === "string" ? (ns.plan as string) : null;
            if (nsPlan) {
              const idx = planOrder.indexOf(nsPlan.toLowerCase());
              const bestIdx = bestPlan ? planOrder.indexOf(bestPlan.toLowerCase()) : -1;
              if (idx > bestIdx) bestPlan = nsPlan;
            }
          }
        }
      } catch { /* ignore timeout/error */ }
    ...
    const planLabels: Record<string, string> = {
      ...
      silver: "Silver",
      premium: "Premium",
      ultimate: "Ultimate - Duo Pro",
    };
    const planLabel = bestPlan
      ? (planLabels[bestPlan.toLowerCase()] || bestPlan)
      : hasReadApi
        ? "Unknown"
        : "Duo - re-authorize for plan details";

    const quotas: JsonRecord = {};

    // Only probe credit endpoints if we have read_api scope
    if (hasReadApi) {
      for (const ep of ["/api/v4/duo/usage", "/api/v4/ai_actions/usage"]) {
        try {
          ...
          if (res.ok) {
            ...
            const used = typeof data.used === "number" ? (data.used as number) : null;
            ...
            if (limit !== null && limit > 0) {
              const rem = remaining ?? (used !== null ? limit - used : null);
              quotas["AI Actions Credits"] = {
                used: used ?? (rem !== null ? limit - rem : 0),
                total: limit,
                remaining: rem ?? 0,
                remainingPercentage: rem !== null ? (rem / limit) * 100 : 0,
                unlimited: false,
              };
              break;
            }
          }
        } catch { /* endpoint may not exist or timeout */ }
      }
    }

    const result: JsonRecord = { plan: planLabel, quotas };
    if (Object.keys(quotas).length === 0) {
      result.message = hasReadApi
        ? `GitLab Duo: ${planLabel}. Usage tracked per request.`
        : `GitLab Duo: ${planLabel}`;
    }
    return result;
  } catch (error) {
    return { message: `GitLab Duo connected. Unable to fetch usage: ${(error as Error).message}` };
  }
}
