...
export const REFRESH_LEAD_MS: Record<string, number> = {
  ...
  "gitlab-duo": 30 * 60 * 1000, // 30 min — GitLab Duo tokens expire in 2h, refresh early to avoid refresh_token expiry
...
export async function refreshGitLabDuoToken(
  ...
  // client_id/client_secret from providerSpecificData (stored at login) or fall back to PROVIDERS config/env
  const clientId =
    ...
    "";
  const clientSecret =
    (providerSpecificData?.clientSecret as string) ||
    PROVIDERS["gitlab-duo"]?.clientSecret ||
