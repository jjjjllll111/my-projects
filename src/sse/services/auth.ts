...
async function maybeSyntheticNoAuthFallback(providerId: string, excludedConnectionIds: Set<string>) {
  ...
  const creds = buildSyntheticNoAuthCredentials();
  try {
    ...
    if (conn?.providerSpecificData && typeof conn.providerSpecificData === "object") {
      creds.providerSpecificData = conn.providerSpecificData as any;
    }
  } catch { /* ignore */ }
  return creds;
}
