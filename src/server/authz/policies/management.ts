...
/**
 * ProxyManager internal calls bypass — server-side calls to
 * /api/settings/free-proxies/live and /api/settings/proxies carry
 * X-Internal-Call: omniroute-server and originate from localhost.
 * Without this, middleware rejects them before route-level bypass.
 */
function isProxyManagerInternalCall(ctx: PolicyContext): boolean {
  if (ctx.request.headers?.get?.("X-Internal-Call") !== "omniroute-server") return false;
  // Accept either: (a) peer-stamped loopback, or (b) Host header localhost
  if (isLoopbackRequest(ctx)) return true;
...
export const managementPolicy: RoutePolicy = {
  ...
  async evaluate(ctx: PolicyContext): Promise<AuthOutcome> {
    ...
    if (isProxyManagerInternalCall(ctx)) {
      return allow({ kind: "management_key", id: "proxy-manager", label: "internal-proxy-manager" });
    }
