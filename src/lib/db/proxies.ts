...
function toRegistryProxyResolution(row: unknown, level: ProxyScope, levelId: string | null) {
  ...
  const relayAuth = (record.type === "vercel" || record.type === "cloudflare") ? extractRelayAuth(record.notes) : undefined;
  return {
    proxy: {
      type: record.type,
      host: record.host,
...
export function redactProxySecrets(proxy: ProxyRegistryRecord): ProxyRegistryRecord {
  ...
  if ((proxy.type === "vercel" || proxy.type === "cloudflare") && proxy.notes) {
    try {
