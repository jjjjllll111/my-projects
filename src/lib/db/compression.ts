...
function normalizeLlmlinguaConfig(value: unknown): LlmlinguaConfig {
  const record = toRecord(value);
  return {
    ...DEFAULT_LLMLINGUA_CONFIG,
    enabled:
      typeof record.enabled === "boolean" ? record.enabled : DEFAULT_LLMLINGUA_CONFIG.enabled,
    model:
      typeof record.model === "string" && record.model.trim()
        ? record.model.trim()
        : DEFAULT_LLMLINGUA_CONFIG.model,
    minTokens: boundedInt(record.minTokens, DEFAULT_LLMLINGUA_CONFIG.minTokens ?? 2000, 0, 100000),
    compressionRate: boundedNumber(
      record.compressionRate,
      DEFAULT_LLMLINGUA_CONFIG.compressionRate ?? 0.5,
      0.1,
      0.9
    ),
    modelPath:
      typeof record.modelPath === "string"
        ? record.modelPath.trim()
        : DEFAULT_LLMLINGUA_CONFIG.modelPath,
  };
}
  ...
  for (const row of rows) {
    ...
    switch (key) {
      ...
      case "contextEditing":
        ...
        break;
      case "llmlingua":
        config.llmlingua = normalizeLlmlinguaConfig(parsed);
        break;
      case "engines":
