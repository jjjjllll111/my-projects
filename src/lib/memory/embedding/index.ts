...
/**
 * List rerank providers, marking which ones have a configured API key.
 * Uses the rerank registry (not embedding models) for Memory Engine rerank UI.
 */
export async function listRerankProviders(): Promise<EmbeddingProviderListing[]> {
  const result: EmbeddingProviderListing[] = [];
