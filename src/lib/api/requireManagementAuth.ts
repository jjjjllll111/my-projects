...
export async function requireManagementAuth(request: Request): Promise<Response | null> {
  // Allow internal server-side calls (ProxyManager, etc.) to bypass auth
  const isInternalCall = request.headers.get('X-Internal-Call') === 'omniroute-server';
  if (isInternalCall) {
    const url = new URL(request.url);
    if (url.hostname === 'localhost' || url.hostname === '127.0.0.1') {
      return null; // Internal call from server-side code, skip auth
    }
