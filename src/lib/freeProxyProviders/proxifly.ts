...
export class ProxiflyProvider implements FreeProxyProvider {
  ...
  async fetchLive(filters: {
    ...
    const quantity = filters.limit || 20;
    const anonymity = process.env.FREE_PROXY_PROXIFLY_ANONYMITY || DEFAULT_ANONYMITY;
    ...
    try {
      const url = new URL(DEFAULT_API_URL);
      url.searchParams.set("format", "json");
      url.searchParams.set("quantity", String(Math.min(quantity, MAX_BATCH_QUANTITY)));
      url.searchParams.set("protocol", filters.protocol || DEFAULT_PROTOCOL);
      url.searchParams.set("anonymity", anonymity);
      ...
      if (!res.ok) {
        console.error(`[ProxiflyProvider] fetchLive failed: HTTP ${res.status}`);
        return [];
      }
      ...
      for (const p of proxies) {
        ...
        // 应用过滤条件
        if (filters.country && item.countryCode !== filters.country.toUpperCase()) {
          continue;
        }
        if (filters.minQuality != null && (item.qualityScore ?? 0) < filters.minQuality) {
          continue;
        }
      ...
      console.log(`[ProxiflyProvider] fetchLive returned ${items.length} proxies`);
      return items;
    } catch (error) {
      console.error("[ProxiflyProvider] fetchLive error:", error);
      return [];
    }
  }
  async list(filters: {
