...
import { TheOldLlmConfigForm } from "../components/TheOldLlmConfigForm";
...
export default function ProviderDetailPageClient() {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {/* TheOldLLM proxy pool config */}
      {providerId === "theoldllm" && (
        <TheOldLlmConfigForm
          providerId={providerId}
          initialConfig={{
            useProxyPool: connections[0]?.providerSpecificData?.useProxyPool ?? 
              (typeof window !== "undefined" && localStorage.getItem("theoldllm-config") 
                ? JSON.parse(localStorage.getItem("theoldllm-config")).useProxyPool 
                : undefined),
            proxyConcurrency: connections[0]?.providerSpecificData?.proxyConcurrency ?? 
              (typeof window !== "undefined" && localStorage.getItem("theoldllm-config") 
                ? JSON.parse(localStorage.getItem("theoldllm-config")).proxyConcurrency 
                : undefined)
          }}
          onSave={async (config) => {
            try {
              const response = await fetch(`/api/providers/theoldllm`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ providerSpecificData: config }),
              });
              ...
              if (typeof window !== "undefined") {
                localStorage.setItem("theoldllm-config", JSON.stringify(config));
              }
              ...
              if (result.autoCreated) {
                useNotificationStore.getState().addNotification({
                  type: "success",
                  message: "配置已保存（已自动创建默认连接）",
                });
              } else {
                useNotificationStore.getState().addNotification({
                  type: "success",
                  message: `配置已保存并应用到 ${result.updatedCount} 个连接`,
                });
              }
              
              // Refresh connections to show the auto-created or updated connection
