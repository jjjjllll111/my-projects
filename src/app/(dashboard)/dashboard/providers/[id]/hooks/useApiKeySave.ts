...
export function useApiKeySave({
  ...
  const handleSaveApiKey = useCallback(
    async (formData: Record<string, unknown>) => {
      try {
        // For theoldllm, check if there's pending config in localStorage
        let requestBody = { provider: providerId, ...formData };
        
        if (providerId === "theoldllm" && typeof window !== "undefined") {
          const pendingConfig = localStorage.getItem("theoldllm-config");
          if (pendingConfig) {
            try {
              const config = JSON.parse(pendingConfig);
              requestBody = {
                ...requestBody,
                providerSpecificData: config,
              };
              // Keep config in localStorage for future connections
            } catch (err) {
              console.warn("Failed to parse theoldllm pending config:", err);
            }
          }
        }
