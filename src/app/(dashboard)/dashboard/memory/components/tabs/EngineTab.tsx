...
export default function EngineTab() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    Promise.all([
      fetch("/api/memory/embedding-providers").then((r) => (r.ok ? r.json() : null)),
      fetch("/api/memory/rerank-providers").then((r) => (r.ok ? r.json() : null)),
    ])
      .then(([embeddingData, rerankData]) => {
        if (cancelled) return;
        if (embeddingData?.providers) setProviders(embeddingData.providers);
        if (rerankData?.providers) setRerankProviders(rerankData.providers);
      })
      .catch(() => {});
    return () => {
      cancelled = true;
