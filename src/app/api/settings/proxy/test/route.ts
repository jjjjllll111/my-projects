...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Vercel/Cloudflare Relay: test by hitting ipify via the relay headers
    if (proxyType === "vercel" || proxyType === "cloudflare") {
      const relayHost = proxy.host;
      // relayAuth lives in notes JSON: { relayAuth } stored by the vercel-deploy or cloudflare-deploy route.
