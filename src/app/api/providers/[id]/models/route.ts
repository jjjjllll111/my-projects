...
function buildTraeModelHeaders(accessToken: string, psd: JsonRecord): Record<string, string> {
  return {
    Authorization: accessToken.startsWith("Cloud-IDE-JWT ")
      ? accessToken
      : `Cloud-IDE-JWT ${accessToken}`,
    Accept: "application/json",
    "X-Trae-Client-Type": "web",
    "X-Preferenced-Language": toNonEmptyString(psd.appLanguage) || "en",
    "x-user-region": toNonEmptyString(psd.userRegion) || "US",
...
async function fetchTraeRemoteModels(args: {
  accessToken: string;
  providerSpecificData: unknown;
  proxy: unknown;
}): Promise<Array<{ id: string; name: string; supportsVision?: boolean }>> {
  const psd = asRecord(args.providerSpecificData);
  const registryBase = getRegistryEntry("trae")?.baseUrl;
  const baseUrl =
    typeof registryBase === "string" && registryBase.trim()
      ? registryBase.trim().replace(/\/+$/, "")
      : "https://core-normal.trae.ai/api/remote/v1";
  const response = await safeOutboundFetch(`${baseUrl}/models`, {
    ...SAFE_OUTBOUND_FETCH_PRESETS.modelsDiscovery,
    guard: getProviderOutboundGuard(),
    proxyConfig: args.proxy as any,
    method: "GET",
    headers: buildTraeModelHeaders(args.accessToken, psd),
  });
  const text = await response.text();
  if (!response.ok) {
    const err = new Error(
      `Trae models API failed: HTTP ${response.status}: ${text.slice(0, 200)}`
  ...
  try {
    ...
    throw new Error("Trae models API returned non-JSON response");
  }
  if (data?.code !== 0) {
    throw new Error(`Trae models API error: ${JSON.stringify(data).slice(0, 300)}`);
  }
  const out: Array<{ id: string; name: string; supportsVision?: boolean }> = [
    { id: "auto", name: "Auto (Code · Server Picks)" },
    { id: "work", name: "Work (Auto · fast)" },
  ];
  const seen = new Set(out.map((m) => m.id));
  for (const group of Array.isArray(data?.data?.list) ? data.data.list : []) {
      ...
      out.push({
        ...
        name:
          typeof model?.display_name === "string" && model.display_name.trim()
            ...
            : id,
        ...(typeof model?.multimodal === "boolean" ? { supportsVision: model.multimodal } : {}),
      });
    }
  }
  return out;
  ...
  try {
    ...
    if (!connection) {
      ...
      if (isNoAuthProvider) {
        ...
        // Special handling for theoldllm — live website scraping
        if (id === "theoldllm") {
          try {
            const discovery = await fetchTheOldLlmModels();
            ...
            const visible = excludeHidden
              ? models.filter((m) => !getModelIsHidden(id, m.id))
              : models;
            ...
            return NextResponse.json({
              ...
              ...(discovery.error && { warning: discovery.error }),
            });
          } catch (error) {
            console.error("[models-api] theoldllm discovery error:", error);
            // Fall through to modelsUrl / local_catalog handling
          }
        }
    ...
    if (provider === "trae") {
      ...
      if (!accessToken) {
        const fallback = buildDiscoveryFallbackResponse({
          cacheWarning: "Trae token unavailable — using cached catalog",
          localWarning: "Trae token unavailable — using local catalog",
        });
        if (fallback) return fallback;
        return NextResponse.json({ error: "Trae token unavailable" }, { status: 401 });
      }

      try {
        const models = await fetchTraeRemoteModels({
          accessToken,
          providerSpecificData: connection.providerSpecificData,
          proxy,
        });
        ...
        const status = (error as any)?.status || getSafeOutboundFetchErrorStatus(error);
        const fallback = buildDiscoveryFallbackResponse({
          cacheWarning:
            status === 401 || status === 403
              ? `Trae auth failed (${status}) — using cached catalog`
              : "Trae models API unavailable — using cached catalog",
          localWarning:
            status === 401 || status === 403
              ? `Trae auth failed (${status}) — using local catalog`
              : "Trae models API unavailable — using local catalog",
        });
        if (fallback) return fallback;
        if (status === 401 || status === 403) {
          return NextResponse.json({ error: `Trae auth failed: ${status}` }, { status });
        }
        return NextResponse.json(
          {
            error: sanitizeErrorMessage((error as Error)?.message) || "Trae models API unavailable",
          },
          { status: status || 502 }
        );
      }
