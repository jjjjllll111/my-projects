...
export default function TraeAuthModal({
  ...
  const handleImportCallbackUrl = async () => {
    if (!callbackUrl.trim()) {
    ...
    setError(null);
    try {
      const res = await fetch("/api/oauth/trae/callback-import", {
        method: "POST",
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="bg-emerald-50 dark:bg-emerald-900/20 p-4 rounded-lg border border-emerald-200 dark:border-emerald-800">
          <p className="text-sm text-emerald-900 dark:text-emerald-200 mb-2">
            Authorize via <span className="font-mono">trae.ai</span> in a popup. On cloud, Trae may end at a 127.0.0.1 callback; copy that full URL into the box below.
          </p>
        ...
        <div className="bg-amber-50 dark:bg-amber-900/20 p-3 rounded-lg border border-amber-200 dark:border-amber-800">
          <p className="text-sm text-amber-900 dark:text-amber-200 mb-2">
            If Trae opens a <span className="font-mono">127.0.0.1/authorize</span> callback instead of returning to this cloud Space, copy the full callback URL from the address bar and paste it here.
          </p>
          <textarea
            value={callbackUrl}
            onChange={(e) => setCallbackUrl(e.target.value)}
            placeholder="http://127.0.0.1:443/authorize?..."
            rows={2}
            className="w-full px-3 py-2 text-sm font-mono border border-border rounded-lg bg-background focus:outline-none focus:border-primary resize-none mb-2"
          />
          <Button onClick={handleImportCallbackUrl} fullWidth disabled={importing || !callbackUrl.trim()}>
            {importing ? "Importing…" : "Import Callback URL"}
          </Button>
        </div>
