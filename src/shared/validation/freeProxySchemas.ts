...
export const cloudflareDeploySchema = z.object({
  apiToken: z
    .string()
    .min(40, "Cloudflare API token looks too short"),
  accountId: z
    .string()
    .min(20, "Cloudflare account ID looks too short")
    .regex(
      /^[a-z0-9]+$/i,
      "Cloudflare account ID must be alphanumeric"
    ),
  workerName: z
    .string()
    .min(1, "Worker name is required")
    .max(63, "Worker name must be 63 characters or less")
    .regex(
      /^[a-z0-9-]+$/i,
      "Worker name must be alphanumeric with hyphens"
    ),
});
