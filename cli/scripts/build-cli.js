...
function copyRecursive(src, dest) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // Preserve symlink — dereferencing duplicates closure (tar bomb)
      try {
...
function buildCliPackage() {
  ...
  //          + headroom runtime (auto-starts the headroom proxy with the server).
  ...
  const headroomRuntimeSrc = path.join(appDir, "headroomRuntime.js");
  if (fs.existsSync(headroomRuntimeSrc)) {
    fs.copyFileSync(headroomRuntimeSrc, path.join(cliAppDir, "headroomRuntime.js"));
    console.log("✅ Copied headroomRuntime.js\n");
  }
